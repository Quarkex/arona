select c.codcontenido, rtrim ('"' || c.codsubtipocont || '",' || subtipos.codsubtipos, ',')  codsubtipocont, sbt.descripcion subtipo, c.codcategoria, ctg.NOMBRE categoria, 
...
ci.datos_interes, ci.palabras_clave, c.publicado, c.REF_VPORTAL, nt.tipo, nt.orden, nt.f_pub_original, 
nti.titulo_corto, nti.resumen, nti.fuente,
indic_noticia.CodigosIndicadores, indic_noticia.NombreIndicadores, indic_noticia.EtiquetaIndicadores, indic_noticia.ValoresIndicadoresLista,
indic_noticia.ValoresIndicadores, areasubarea.codareas, areasubarea.areas, areasubarea.codsubareas, areasubarea.subareas,
...
inner join noticia nt on nt.codcontenido = c.codcontenido
inner join noticia_idioma nti on c.codcontenido = nti.codcontenido and ci.codidioma = nti.codidioma
inner join subtipo_contenido_idioma sbt on sbt.CODSUBTIPOCONT = c.CODSUBTIPOCONT and sbt.codidioma = ci.codidioma
inner join categoria_idioma ctg on ctg.codcategoria = c.codcategoria and ci.codidioma = ctg.codidioma
...
group by igc.codcontenido, igc.codidioma) indic_noticia on indic_noticia.codcontenido = c.codcontenido 
and ci.codidioma= indic_noticia.codidioma
