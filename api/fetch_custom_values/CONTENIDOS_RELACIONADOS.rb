if @doc.has_key? 'CODCONTENIDOSRELACIONADOS' then
    ...
    doc_elements.map! { |i|
        ...
        $db.collections.each do | collection |
            element = collection.find({ "IDIOMA": "es", "CODCONTENIDO": i, "MULTIIDIOMA": 1 }).sort("CODCONTENIDO" => -1).first
            element = collection.find({ "IDIOMA": @doc["IDIOMA"], "CODCONTENIDO": i }).sort("CODCONTENIDO" => -1).first if element == nil
            if element != nil
        ...
        if element.has_key? 'IMAGEN' then
            image = $resources.find({ "IDIOMA": "es", "CODRECURSO": element['IMAGEN'], "MULTIIDIOMA": 1 }).sort("RECURSO" => -1).first
            image = $resources.find({ "IDIOMA": @doc["IDIOMA"],"CODRECURSO": element['IMAGEN'] }).sort("RECURSO" => -1).first if image == nil
    ...
    doc_elements.each do | element |
        availible_elements.push(element) if should_add(element) == true unless element == nil
