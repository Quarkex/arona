app.directive('appRelatedContent', function () {
    ...
    var content_a = '<img class="md-media-md card-media" alt="{{ card.name }}" src="http://www.arona.org/portal/imagecache/ficha/{{ card.IMAGEN }}">';
    ...
    content = '<md-card flex md-theme="default" md-theme-watch style="min-height: 150px; background-color: #f6f6f6;" ng-repeat="card in element().CONTENIDOS_RELACIONADOS">' + content + '</md-card>';

    content = '<div layout="row" layout-wrap flex="100" layout-align="begin strech">' + content + '</div>';
